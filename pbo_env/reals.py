...
class RealEnvironment(BaseEnvironment):
    def init(self) -> Tuple[
        ...
        action = {'problem': copy.deepcopy(self.get_fixed_problem(opts))}
    ...
    def get_fixed_problem(self, opts):
        train_set, train_pro_id = get_train_set_cec21(opts)
        train_set_dict = dict(zip(train_pro_id, train_set))
        return train_set_dict[opts.problem]
    ...
    def pure_opt_reward(self, pre_stu_pop, next_stu_pop):
        ...
        r = (pre_stu_pop.gbest_cost-next_stu_pop.gbest_cost) / \
            (pre_stu_pop.init_cost-pre_stu_pop.problem.optimum)
        return r

    def neg_opt_reward(self, stu_pop):
        assert stu_pop.init_cost > stu_pop.problem.optimum, 'error: init cost == problem.optimum!!'
        r = -(stu_pop.gbest_cost-stu_pop.problem.optimum) / \
            (stu_pop.init_cost-stu_pop.problem.optimum)
    ...
    def get_reward(self, prepopulation, population):
        r = self.pure_opt_reward(prepopulation, population) + \
            0.5 * self.neg_opt_reward(population)
        return r
