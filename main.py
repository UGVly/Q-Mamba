...
def run(cfg):
    ...
    if cfg.mode == 'train' and cfg.resume == None:
        cfg.checkpoint_folder = cfg.checkpoint_folder + \
            '/' + cfg.model + '/' + current_time

    if not cfg.no_tb:
        tb_logger = SummaryWriter(os.path.join(
            cfg.log_dir, "{}D".format(cfg.dim), cfg.model))
    else:
        tb_logger = None

    # cfg.replay_memories_folder = cfg.replay_memories_folder + '/' +  "{}D".format(cfg.dim) + cfg.model
    ...
    if cfg.create_replay_memories:
        
        cfg.replay_memories_folder = cfg.replay_memories_folder+'/'+ cfg.model + '/' + \
            "{}D@{}steps@{}episodes@{}num_actions@".format(
                cfg.dim, cfg.max_num_steps_per_episode, cfg.num_episodes,cfg.num_actions) + current_time
    ...
    if cfg.mode == 'train':
        ...
        q_learner = QLearner(
            ...
            dataset=ReplayMemoryDataset(
                replay_memories_folder=cfg.replay_memories_folder),
